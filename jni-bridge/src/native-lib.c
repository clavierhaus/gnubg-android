...
/* getCandidates: returns a flat int array with layout:
 *   [0]        = n  (number of candidates written)
 *   [1 + i*9]  = anMove[0..7] for candidate i   (8 ints)
 *   [1 + i*9 + 8] = Float.floatToRawIntBits(equity_i)  (1 int)
 *
 * Kotlin unpacks with: n = arr[0]; for i in 0..<n: move = arr[1+i*9..1+i*9+7],
 * equity = Float.fromBits(arr[1+i*9+8]).
 *
 * nMax is capped at 20 (sufficient for any realistic blunder/difficulty use).
 */
JNIEXPORT jintArray JNICALL
Java_com_clavierhaus_gnubg_Engine_getCandidates(JNIEnv *env, jobject thiz,
                                                 jintArray jboard,
                                                 jint die0, jint die1,
                                                 jint nMax) {
    (void)thiz;
    if (!gnubg_initialised) {
        jintArray r = (*env)->NewIntArray(env, 1);
        jint zero = 0;
        (*env)->SetIntArrayRegion(env, r, 0, 1, &zero);
        return r;
    }

    int cap = (int)nMax;
    if (cap <= 0 || cap > 20) cap = 20;
    ...
    int   *moves    = (int *)   malloc(cap * 8 * sizeof(int));
    float *equities = (float *) malloc(cap     * sizeof(float));
    if (!moves || !equities) {
        free(moves); free(equities);
        jintArray r = (*env)->NewIntArray(env, 1);
        jint zero = 0;
        (*env)->SetIntArrayRegion(env, r, 0, 1, &zero);
        return r;
    ...
    int n = gnubg_mobile_get_candidates(in, (int)die0, (int)die1,
                                        moves, equities, cap);
    if (n < 0) n = 0;

    /* Result: 1 header int + n * 9 ints */
    int total = 1 + n * 9;
    jintArray result = (*env)->NewIntArray(env, total);
    jint *buf = (jint *) malloc(total * sizeof(jint));
    if (!buf) {
        free(moves); free(equities);
        jint zero = 0;
        (*env)->SetIntArrayRegion(env, result, 0, 1, &zero);
        return result;
    }
    ...
    for (int i = 0; i < n; i++) {
        int base = 1 + i * 9;
        for (int j = 0; j < 8; j++)
            buf[base + j] = (jint) moves[i * 8 + j];
        /* Pack the float equity as raw bits for lossless transport. */
        union { float f; int32_t i; } u;
        u.f = equities[i];
        buf[base + 8] = (jint) u.i;
    }
    (*env)->SetIntArrayRegion(env, result, 0, total, buf);

    free(buf); free(moves); free(equities);
    return result;
}
