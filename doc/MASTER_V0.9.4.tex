...
\hypersetup{
  ...
  linkcolor=ruleblue,
  urlcolor=ruleblue,
  pdftitle={GNU Backgammon Android Port -- MASTER v0.9.4},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{gray}GNU Backgammon Android Port --- MASTER v0.9.4}
...
\section{GNU Backgammon Android Port --- MASTER v0.9.4}\label{gnu-backgammon-android-port-master-v09}
...
\subsection{11. Version v0.9.1 --- Basic UI
Complete}\label{version-v091-basic-ui-complete}

\subsubsection{11.1 Milestone}\label{milestone-v091}

Version v0.9.1 marks the completion of the basic Android UI layer. The
app is fully functional as a visual shell: the board renders correctly
on any screen size and aspect ratio, all 24 points are geometrically
accurate, checkers are placed in the starting position, dice and cube
are rendered, and the settings system is fully wired to a
\texttt{GameViewModel}. The next phase is engine integration --- wiring
the JNI bridge to live gameplay.

\subsubsection{11.2 UI Components
Delivered}\label{ui-components-delivered}

\paragraph{Board (\texttt{Board.kt})}\label{boardkt}

\begin{itemize}
\tightlist
\item
  Geometry derived directly from \texttt{boarddim.h} --- 24 points,
  asymmetric layout with single right-side bearoff tray, narrower
  centre bar
\item
...
\item
  Checkers in starting position --- size governed by triangle height,
  5 checkers = triangle height
\item
  Side-on rectangle representation for bearoff tray checkers (15 per
  tray)
\item
...
\item
  Two distinct bearoff trays (top/bottom) with white inner outline and
  dark outer border
\end{itemize}

\paragraph{Layout (\texttt{GameLayout.kt})}\label{gamelayoutkt}
...
\item
  Settings gear icon (Material Icons) top-left, safe from screen
  rounded corners
\item
  Full-screen settings overlay on tap
\end{itemize}

\paragraph{Settings System}\label{settings-system}

\begin{itemize}
\tightlist
\item
  \texttt{GameViewModel} (\texttt{engine/GameViewModel.kt}) ---
  single source of truth for all settings and future game state,
  using \texttt{StateFlow}
\item
  \texttt{GameSettings} data class with 16 typed fields covering all
  four settings categories
\item
  \texttt{SettingsScreen.kt} --- full-screen BMPCC-inspired settings
  UI: horizontal tab bar (GAME / BOARD / ENGINE / ANALYSIS),
  scrollable sections, live-wired toggles and controls
\item
  All switches toggle immediately and propagate to the board
\item
  Match length and automatic doubles use +/-2/+/-1 steppers
\item
  Difficulty selector (Beginner/Intermediate/Advanced/Expert) maps to
  gnubg ply depth
\item
  Error classification thresholds displayed (editable in next phase)
\end{itemize}

\paragraph{Theme Infrastructure}\label{theme-infrastructure}

\begin{itemize}
\tightlist
\item
  \texttt{BoardTheme} enum: OCEAN / CLASSIC / FOREST
\item
  \texttt{Difficulty} enum with ply mapping
\item
  Settings UI radio buttons wired to \texttt{GameViewModel} ---
  colour theme switching renders in next phase
\end{itemize}

\paragraph{Build Environment}\label{build-environment-v091}

\begin{itemize}
\tightlist
\item
  SDKMAN manages Java 21.0.7-tem and Gradle 8.14 via
  \texttt{.sdkmanrc}
\item
  \texttt{local.properties} points to
  \texttt{/home/erweitert/gnubg-android/android-sdk}
\item
  \texttt{gradle.properties}: \texttt{android.useAndroidX=true},
  \texttt{android.suppressUnsupportedCompileSdk=35}
\item
  KDE Connect established as the screenshot-to-desktop workflow
\item
  16KB ELF alignment warning noted --- to be resolved in engine
  rebuild phase with \texttt{-Wl,-z,max-page-size=16384}
\end{itemize}

\subsubsection{11.3 Immediate Next Steps
(v0.9.2)}\label{immediate-next-steps-v092}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Implement colour theme switching --- OCEAN/CLASSIC/FOREST palettes
  derived from Material You or fixed grayscale
\item
  Wire engine initialisation --- call \texttt{Engine.initialise()}
  with weights path on app start
\item
  Implement \texttt{BoardState.kt} --- live board encoding from
  engine \texttt{matchstate}
\item
  Wire \texttt{Engine.findBestMove()} and \texttt{Engine.rollout()}
  to the game loop via \texttt{GameViewModel}
\item
  Implement touch input for checker moves --- tap point to select,
  tap destination to move
\item
  Wire \texttt{Engine.cubeDecision()} to the doubling cube UI
\item
  Rebuild engine \texttt{.so} files with 16KB page alignment
\item
  Populate player panel (left 25\%) with score, match state, timer
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{12. Version v0.9.2 --- Settings System \& Theme
Persistence}\label{version-v092-settings-theme}

\subsubsection{12.1 Milestone}\label{milestone-v092}

Version v0.9.2 completes the settings infrastructure. All settings are
live-wired to a \texttt{GameViewModel}, the board colour theme persists
across app restarts via \texttt{DataStore}, and a fourth theme option
(System) exposes Android Material You dynamic colour derived from the
device wallpaper. The app is now a complete visual shell with persistent
user preferences, ready for engine integration in v0.9.3.

\subsubsection{12.2 Delivered}\label{delivered-v092}

\paragraph{Colour Themes}\label{colour-themes-v092}

Four board themes are available and selectable in Settings → Board:

\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
Theme & Seed & Character \
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Ocean & Fixed \texttt{\#1565C0} & Deep blue --- default \
Classic & Fixed grayscale & Black, white, and grays \
Forest & Fixed \texttt{\#2E7D32} & Deep green \
System & Device wallpaper & Material You dynamic colour \
\end{longtable}

All colour roles (board field, triangles, bar, bearoff trays, checkers,
dice, cube, pip text, frame) are encapsulated in a \texttt{BoardPalette}
data class. The \texttt{BoardPalettes} object provides static instances
for Ocean, Classic, and Forest; the System theme is resolved at
composition time from \texttt{MaterialTheme.colorScheme}.

The Classic theme uses the light-gray triangle colour (\texttt{\#444444})
as the checker rim and dice colour for both players, giving a coherent
monochromatic grayscale aesthetic without excessive contrast.

Bearoff tray checkers use increased inner padding (\texttt{slotPad = ol
* 3f}) to prevent visual attachment to the tray outline.

\paragraph{Theme Persistence
(\texttt{PreferencesManager.kt})}\label{theme-persistence}

Board theme selection persists across app restarts using
\texttt{androidx.datastore:datastore-preferences:1.1.1}. The
\texttt{PreferencesManager} object exposes:

\begin{itemize}
\tightlist
\item
  \texttt{boardThemeFlow(context)} --- \texttt{Flow<BoardTheme>} read
  on \texttt{GameViewModel} init
\item
  \texttt{saveBoardTheme(context, theme)} --- called on every theme
  change via \texttt{coroutineScope.launch}
\end{itemize}

The \texttt{GameViewModel} was promoted from \texttt{ViewModel} to
\texttt{AndroidViewModel} to access the \texttt{Application} context
required by \texttt{DataStore}.

\paragraph{Orientation}\label{orientation-v092}

\texttt{SCREEN\_ORIENTATION\_SENSOR\_LANDSCAPE} replaces the previous
\texttt{SCREEN\_ORIENTATION\_LANDSCAPE}, honouring 180° rotation when
the device is physically flipped. Portrait is never permitted.

\paragraph{Gear Icon Positioning}\label{gear-icon-v092}

The settings gear icon (Material Icons \texttt{Settings}) is positioned
at \texttt{padding(start = 16.dp, top = 12.dp)} --- clear of the Pixel
8 Pro rounded screen corners and visible in both landscape orientations.

\subsubsection{12.3 Architecture
Summary}\label{architecture-summary-v092}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{engine/GameViewModel.kt} & Single source of truth for all settings; \texttt{AndroidViewModel} with \texttt{StateFlow} \
\texttt{engine/GameSettings.kt} & 16-field data class for all user preferences \
\texttt{engine/PreferencesManager.kt} & DataStore read/write for persistent theme \
\texttt{ui/BoardPalette.kt} & \texttt{BoardPalette} data class + \texttt{BoardPalettes} static instances \
\texttt{ui/Board.kt} & Consumes \texttt{GameSettings}; resolves System theme from \texttt{MaterialTheme} \
\texttt{ui/GameLayout.kt} & Passes \texttt{GameSettings} to board and settings screen \
\texttt{ui/SettingsScreen.kt} & Four-tab settings UI wired to \texttt{GameViewModel} \
\end{longtable}

\subsubsection{12.4 Immediate Next Steps
(v0.9.3)}\label{immediate-next-steps-v093}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Wire \texttt{Engine.initialise()} on app start --- extract neural net
  weights from assets to device storage
\item
  Implement \texttt{BoardState.kt} --- live board encoding bridging
  engine \texttt{matchstate} to Compose state
\item
  Implement touch input --- tap point to select checker, tap
  destination to move
\item
  Wire \texttt{Engine.findBestMove()} to the game loop
\item
  Wire dice rolling --- \texttt{Engine} random roll → animate dice →
  enable legal moves
\item
  Wire \texttt{Engine.cubeDecision()} to doubling cube UI
\item
  Rebuild \texttt{.so} files with \texttt{-Wl,-z,max-page-size=16384}
  for Android 16 16KB page alignment
\item
  Populate player panel with score, match state, player name, timer
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{13. Version v0.9.3 --- Engine Integration \& Dice
Rolling}\label{version-v093-engine-integration}

\subsubsection{13.1 Milestone}\label{milestone-v093}

Version v0.9.3 achieves the first live engine integration milestone: the
gnubg neural net evaluation engine initialises successfully on app
start, dice roll correctly on each tap of the Roll button, and the
board checker positions are correctly mapped from the engine board
encoding to the UI coordinate system. The app runs stably without
crashes across multiple Roll taps.

\subsubsection{13.2 Delivered}\label{delivered-v093}

\paragraph{Engine Initialisation}\label{engine-init-v093}

\begin{itemize}
\tightlist
\item
  \texttt{AssetExtractor.kt} extracts \texttt{gnubg.weights} from the
  APK assets to \texttt{context.filesDir} on first run
\item
  \texttt{Engine.initialise(weightsPath)} called on a dedicated
  \texttt{gnubg-engine-thread} via \texttt{Executors.newSingleThreadExecutor}
\item
  \texttt{\_engineReady} \texttt{StateFlow<Boolean>} gates the Roll
  button --- it only appears after the engine has fully initialised
\item
  Engine initialisation confirmed via \texttt{gnubg-jni: Engine
  initialised} logcat entry
\end{itemize}

\paragraph{New JNI Entry Points}\label{new-jni-v093}

Four new functions added to \texttt{native-lib.c} and declared in
\texttt{Engine.kt}:
...
\bottomrule\noalign{}
\endlastfoot
\texttt{Engine.rollDice()} & Returns \texttt{IntArray[2]} with random dice values (1--6) via \texttt{rand()} \
\texttt{Engine.getLegalMoves(board, d0, d1)} & Returns best move as \texttt{IntArray[8]} via \texttt{FindBestMoveNoLocking} \
\texttt{Engine.isGameOver(board)} & Returns 0/1/2 for in-progress/player-1-wins/player-0-wins \
\texttt{Engine.newGame()} & Returns standard starting position as \texttt{IntArray[50]} via \texttt{InitBoard} \
\end{longtable}

\paragraph{Game State Architecture}\label{game-state-v093}

\begin{itemize}
\tightlist
\item
  \texttt{BoardState.kt} --- immutable data class holding board
  encoding, turn, dice, legal moves, selected point, cube state, pip
  counts, game phase, and winner
\item
  \texttt{GamePhase} enum: \texttt{WAITING\_FOR\_ROLL},
  \texttt{HUMAN\_MOVING}, \texttt{ENGINE\_THINKING},
  \texttt{GAME\_OVER}
\item
  \texttt{GameViewModel} extended with \texttt{\_gameState:
  MutableStateFlow<BoardState>} and game loop functions:
  \texttt{rollDice()}, \texttt{selectPoint()}, \texttt{moveTo()},
  \texttt{engineMove()}
\item
  Pip count calculation via \texttt{calcPips(board, player)}
\end{itemize}

\paragraph{Board Rendering from Live
State}\label{board-rendering-v093}

\begin{itemize}
\tightlist
\item
  \texttt{BackgammonBoard} now accepts \texttt{BoardState} and renders
  checkers from live engine board encoding
\item
  Dice rendered from \texttt{gameState.dice} --- human dice (light) in
  right half, engine dice (dark) in left half
\item
  Pip counts rendered from \texttt{gameState.pipCountHuman} and
  \texttt{gameState.pipCountEngine}
\item
  Selected point highlighted with semi-transparent white overlay
\end{itemize}

\paragraph{Player Panel}\label{player-panel-v093}

Left 25\% panel now shows:
\begin{itemize}
\tightlist
\item
  Roll button (only when engine ready, human turn, waiting for roll)
\item
  Thinking\ldots{}'' indicator during engine turn
\item
  Game over message (You win!'' / Engine wins!'')
\item
  Turn indicator
\end{itemize}

\subsubsection{13.3 Challenges and Solutions}\label{challenges-v093}

This version required resolving a deep thread-safety issue in the gnubg
engine's thread-local storage (TLS) system. The problem, investigation,
and fix are documented here in full as they represent a significant
engineering finding.

\paragraph{The Problem}\label{tls-problem}

On every tap of Roll, the app crashed with \texttt{SIGSEGV} at
\texttt{FindnSaveBestMovesNoLocking+152}, fault address
\texttt{0x0000000000000008} (a null pointer dereference at offset 8).

\paragraph{Root Cause Investigation}\label{tls-investigation}

The crash traced through the call stack:
\texttt{getLegalMoves} → \texttt{FindBestMoveNoLocking} →
\texttt{FindnSaveBestMovesNoLocking} → \texttt{GenerateMoves} →
\texttt{MT\_Get\_aMoves()} → \texttt{td.tld->aMoves}.

The macro \texttt{MT\_Get\_aMoves()} is defined in
\texttt{multithread.h} (non-multithread path, line 194) as:

\begin{verbatim}
#define MT_Get_aMoves() td.tld->aMoves
\end{verbatim}

\texttt{td} is a global \texttt{ThreadData} struct. \texttt{td.tld} is
a single global pointer to a \texttt{ThreadLocalData} struct containing
pre-allocated move buffers (\texttt{aMoves}) and neural net state
(\texttt{pnnState}).

The engine's \texttt{TLSGet()} function (in \texttt{stubs.c}) uses
\texttt{GPrivate} (GLib's thread-local storage) to lazily allocate a
\texttt{ThreadLocalData} per thread. However, it only set
\texttt{td.tld} once during initialisation --- on the initialisation
thread. When coroutine engine calls ran on a different thread (the
\texttt{gnubg-engine-thread} executor), \texttt{td.tld} still pointed
to the initialisation thread's TLD, or was never updated for the new
thread, causing the null dereference.
...
\tightlist
\item
  \textbf{Adding \texttt{td.tld = tld} in \texttt{TLSGet()}} ---
  insufficient because \texttt{td.tld} is a single global overwritten
  by any thread
\item
  \textbf{\texttt{\_\_thread} shadow variable} --- correct concept but
  the macro expansion in \texttt{eval.c} still bypassed it
\item
  \textbf{Macro override in \texttt{native-lib.c}} --- too late; macros
  are expanded in \texttt{eval.c} at compile time
\item
  \textbf{Dedicated \texttt{HandlerThread}} --- correct thread
  management but \texttt{Handler.asCoroutineDispatcher()} doesn't exist
\item
  \textbf{\texttt{Executors.newSingleThreadExecutor} with named thread}}
  --- correct, but macros still unresolved
\end{enumerate}

\paragraph{The Fix}\label{tls-fix}

The correct fix required two changes:

\textbf{1. Patch \texttt{multithread.h}} to redefine
\texttt{MT\_Get\_aMoves()} and \texttt{MT\_Get\_nnState()} to call
\texttt{TLSGet()} instead of accessing \texttt{td.tld} directly:
...
This ensures that every call to \texttt{MT\_Get\_aMoves()} in
\texttt{eval.c} goes through \texttt{TLSGet()}, which lazily allocates
a \texttt{ThreadLocalData} for the current thread using \texttt{GPrivate}.

\textbf{2. \texttt{TLSGet()} already uses \texttt{GPrivate}} for
per-thread lazy allocation of \texttt{aMoves} and \texttt{pnnState}
buffers (including \texttt{nnContact.cHidden}/\texttt{cInput} sized
neural net state arrays). With the macro fix, this lazy allocation now
runs correctly on any thread that calls an engine function.

\textbf{3. Dedicated engine thread} in \texttt{GameViewModel}:
...
All engine calls (\texttt{initialise}, \texttt{newGame},
\texttt{rollDice}, \texttt{getLegalMoves}, \texttt{findBestMove},
\texttt{applyMove}, \texttt{isGameOver}) are dispatched exclusively on
this thread via \texttt{viewModelScope.launch(engineThread)}.

\paragraph{Board Encoding Correction}\label{board-encoding-v093}

The gnubg board encoding uses a mirrored perspective:
\texttt{anBoard[0][i]} = opponent's checkers at their point \texttt{i+1},
\texttt{anBoard[1][i]} = current player's checkers at their point
\texttt{i+1}. The UI mapping was initially incorrect. The correct
mapping for our layout (human = light, engine = dark):

\begin{verbatim}
// Human (light) checkers at UI point n:
humanCount = board[n - 1]           // anBoard[0][n-1]

// Engine (dark) checkers at UI point n:
engineCount = board[25 + (24 - n)]  // anBoard[1][24-n]
\end{verbatim}

\subsubsection{13.4 Immediate Next Steps
(v0.9.4)}\label{immediate-next-steps-v094}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Implement touch input for checker moves --- tap point to select, tap
  destination to move; wire to \texttt{GameViewModel.selectPoint()} and
  \texttt{moveTo()}
\item
  Fix \texttt{getLegalMoves} to return all legal moves (not just best
  move) --- requires proper match state setup for
  \texttt{GenerateMoves}
\item
  Implement engine auto-roll and move after human completes turn
\item
  Wire \texttt{Engine.cubeDecision()} to doubling cube UI
\item
  Add New Game'' button
\item
  ...
  for Android 16 16KB page alignment
\item
  Populate player panel with score, match length display, player names
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{14. Version v0.9.4 --- Functional One-Point
Game}\label{version-v094-functional-one-point-game}

\subsubsection{14.1 Milestone}\label{milestone-v094}

Version v0.9.4 delivers the first fully playable one-point game. Both
players move correctly, the gnubg engine plays its turn automatically,
all move validation is performed exclusively by the engine, touch input
is complete, and the board renders all game states including bar
checkers and bearing off.

\subsubsection{14.2 Delivered}\label{delivered-v094}

\paragraph{Touch Input}\label{touch-input-v094}

A \texttt{Box} with \texttt{pointerInput(detectTapGestures)} wraps the
board \texttt{Canvas}. On each tap, the board coordinate is converted
from screen pixels to the gnubg coordinate system and matched against
the legal moves list returned by \texttt{GenerateMoves}. No move
computation is done in Kotlin — the engine exclusively determines what
is legal.
...
\tightlist
\item
  \textbf{Tap a point} — the first legal sub-move originating from that
  point is applied using the first remaining die. gnubg's
  \texttt{GenerateMoves} encodes each complete turn as up to four
  sub-moves in a flat \texttt{anMove[8]} array; the UI iterates
  sub-move pairs to find a match.
\item
  \textbf{Tap the dice area} — swaps die order (first die ↔ second
  die), changing which die is consumed first.
\item
  \textbf{Bar re-entry} — when human checkers are on the bar
  (\texttt{anBoard[1][24] > 0}), taps are automatically directed to bar
  re-entry regardless of the tapped point.
\item
  \textbf{Bearing off} — gnubg encodes the bearoff destination as index
  24; this is detected automatically from the legal moves list.
\end{itemize}

\paragraph{Commit and Cancel
Buttons}\label{commit-cancel-v094}

Two buttons appear in the player panel during \texttt{HUMAN\_MOVING}
phase:

\begin{itemize}
\tightlist
\item
  \textbf{Commit} (green ✓) — active only when \texttt{remainingDice}
  is empty or all remaining dice are blocked. Switches turn to engine
  and triggers engine auto-roll after a 500ms delay.
\item
  \textbf{Cancel} (red ✕) — active when \texttt{moveHistory} is
  non-empty. Restores the previous board state and dice list from the
  history stack, then refreshes legal moves from \texttt{getLegalMoves}.
\end{itemize}

Move history is maintained as two parallel stacks in
\texttt{BoardState}: \texttt{moveHistory: List<IntArray>} (board
snapshots) and \texttt{diceHistory: List<List<Int>>} (remaining dice
snapshots). Each tap pushes to both stacks; Cancel pops from both.

\paragraph{Engine Turn}\label{engine-turn-v094}
...
\tightlist
\item
  \texttt{rollDice()} is called automatically on \texttt{engineThread}
\item
  \texttt{Engine.swapBoard(board)} is called to flip \texttt{anBoard[0]}
  and \texttt{anBoard[1]}, placing engine checkers in \texttt{anBoard[1]}
  (gnubg always operates from the moving player's perspective)
\item
  \texttt{Engine.findBestMove(swapped, d0, d1)} finds the best move
  using \texttt{FindBestMoveNoLocking} with \texttt{defaultFilters}
\item
  \texttt{Engine.applyMove(swapped, move)} applies the move
\item
  \texttt{Engine.swapBoard(result)} restores the board to our encoding
\end{enumerate}

\paragraph{New JNI Entry Points}\label{new-jni-v094}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{Engine.swapBoard(board)} & Calls \texttt{SwapSides()} — flips \texttt{anBoard[0]} ↔ \texttt{anBoard[1]} \\
\texttt{Engine.pipCount(board)} & Calls \texttt{PipCount()} — returns \texttt{IntArray[2]} pip counts \\
\texttt{Engine.newGame()} & Calls \texttt{InitBoard()} — returns starting position \\
\end{longtable}

\paragraph{Pip Count}\label{pip-count-v094}

Pip counts are computed by gnubg's \texttt{PipCount()} function via the
\texttt{Engine.pipCount(board)} JNI call. Returns
\texttt{IntArray[2]}: index 0 = \texttt{anBoard[0]} pip count, index 1
= \texttt{anBoard[1]} pip count. In our encoding: index 1 = human
(light), index 0 = engine (dark).

\paragraph{Bar Checker Display}\label{bar-checker-v094}

Bar checkers are read from \texttt{anBoard[0][24]} (engine, shown in
top half of bar) and \texttt{anBoard[1][24]} (human, shown in bottom
half of bar) and rendered as standard checkers centred on the bar.

\paragraph{Dice Display}\label{dice-display-v094}

\begin{itemize}
\tightlist
\item
  Human dice (light) positioned in the right half near the bearoff
  tray
\item
  Engine dice (dark) positioned in the left half near the bar
\item
  Used dice rendered at 35\% opacity; remaining dice at full opacity
\item
  Doubles shown as four dice; normal rolls as two
\end{itemize}

\paragraph{New Game}\label{new-game-v094}

A New Game button appears on game over. \texttt{GameViewModel.newGame()}
calls \texttt{Engine.newGame()} which calls \texttt{InitBoard()} and
resets \texttt{BoardState} to the starting position.

\subsubsection{14.3 Board Encoding
Reference}\label{board-encoding-v094}

gnubg's \texttt{TanBoard anBoard[2][25]} encoding as used in this port:
...
\bottomrule\noalign{}
\endlastfoot
\texttt{anBoard[0][i]} & Engine/dark & UI point \texttt{i+1} \\
\texttt{anBoard[1][i]} & Human/light & UI point \texttt{24-i} \\
\texttt{anBoard[0][24]} & Engine checkers on bar & Bar (top half) \\
\texttt{anBoard[1][24]} & Human checkers on bar & Bar (bottom half) \\
\end{longtable}

Human moves from point 24 (top right) counterclockwise to point 1
(bottom right). Engine moves from point 1 (bottom left) counterclockwise
to point 24 (top left).

\subsubsection{14.4 gnubg Perspective
Rule}\label{gnubg-perspective-v094}

All gnubg engine functions (\texttt{GenerateMoves},
\texttt{FindBestMoveNoLocking}, \texttt{ApplyMove}) operate from the
perspective of the player whose checkers are in \texttt{anBoard[1]}.
In our encoding, human checkers are always in \texttt{anBoard[1]}.
Therefore:

\begin{itemize}
\tightlist
\item
  Human turn: pass board directly — \texttt{anBoard[1]} already contains
  human checkers
\item
  Engine turn: call \texttt{Engine.swapBoard()} before and after each
  engine function call
\end{itemize}

\subsubsection{14.5 Immediate Next Steps
(v0.9.5)}\label{immediate-next-steps-v095}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Player panel content — score, match length, player names, timer
\item
  Doubling cube interaction — offer, accept, pass
\item
  Game over screen with gammon/backgammon detection
\item
  Rebuild \texttt{.so} files with \texttt{-Wl,-z,max-page-size=16384}
  for Android 16 16KB page alignment
\item
  SGF save/load for game history
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{GNU Backgammon Android Port --- MASTER v0.9.4 --- clavierhaus.at
--- June 2026}
