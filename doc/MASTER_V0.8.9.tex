...
\hypersetup{
  ...
  linkcolor=ruleblue,
  urlcolor=ruleblue,
  pdftitle={GNU Backgammon Android Port -- MASTER v0.8.9},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{gray}GNU Backgammon Android Port --- MASTER v0.8.9}
...
\section{GNU Backgammon Android Port --- MASTER v0.8.9}\label{gnu-backgammon-android-port-master-v085}
...
current state of the project as of v0.8.7. It is intended as a working
...
\subsection{5. Version v0.8.7 --- Milestone}\label{version-v085}

\subsubsection{5.1 Assessment}\label{assessment-v085}

Version v0.8.7 delivers a playable one-point game with correct gnubg
...
\subsubsection{5.5 Immediate Next Steps (v0.8.7)}\label{next-steps-v085}
...
\subsection{6. Version v0.8.7 --- Engine Dice, Roll Button, Gameplay Stability}\label{version-v086}

\subsubsection{6.1 Milestone}\label{milestone-v086}

Version v0.8.7 completes the core visual game loop. Engine dice are now
...
\subsection{7. Version v0.8.7 --- Cube, Score, Proportions, Coordinate Fix}\label{version-v087}

\subsubsection{7.1 Milestone}\label{milestone-v087}

Version v0.8.7 completes the visual game loop for single-point play and
lays the groundwork for multi-point matches. The doubling cube is now
interactive, score is tracked and displayed, and the coordinate system
is fixed for correct tap detection across different screen sizes.

\subsubsection{7.2 Delivered}\label{delivered-v087}

\paragraph{Doubling Cube Interaction}

Tapping the cube in the bar area calls \texttt{CommandDouble(NULL)}.
gnubg evaluates the position and either takes or drops. In a 1-point
match the cube is correctly suppressed by gnubg (backgammon rules: no
doubling in a 1-point match). In multi-point matches the cube will work
correctly. The cube tap is detected before the bar checker re-entry
check to avoid interception. A phase guard (\texttt{ENGINE\_THINKING})
and \texttt{AtomicBoolean} prevent duplicate calls from the Xiaomi
hardware delivering multiple simultaneous touch events.

\paragraph{Score Tracking}

\texttt{getMatchScore()} JNI reads \texttt{ms.anScore[0/1]} and
\texttt{ms.nMatchTo} directly. Score is read in every
\texttt{readMatchState} call and stored in \texttt{BoardState}. The
left panel displays engine score, a ``vs'' separator, and human score.
Avatar placeholders (``GNU'' / ``You'' coloured circles) frame the
scores.

\paragraph{Match Length}

\texttt{Engine.newGame(matchLength: Int)} passes the match length to
\texttt{CommandNewMatch(szMatch)}. \texttt{GameSettings.matchLength}
is wired through. UI selector (1, 3, 5, 7 point) is pending.

\paragraph{Gammon/Backgammon Detection}

\texttt{getGameResult()} reads \texttt{plGame->plNext->p->g.fWinner}
and \texttt{nPoints} from the game record. \texttt{nPoints = nCube * n}
where \texttt{n = GameStatus()} returns 1/2/3. The result text
(``You win'', ``You win --- Gammon!'', ``You win --- Backgammon!'')
is derived from \texttt{nPoints} in \texttt{GameLayout}.

\paragraph{Blocked Die Graying}

After rolling, each die is checked against the legal move list: if no
move in \texttt{GenerateMoves} uses that die value as a sub-move step,
the die is added to \texttt{blockedDice} and rendered grayed.
\texttt{blockedDice} is recomputed after each sub-move tap.

\paragraph{Checker Proportions}

Checker radius changed from \texttt{uy(PT\_H)/10f} (hardcoded to
triangle height) to \texttt{ux(PT\_W) * 0.40f} (relative to point
width). Step spacing \texttt{r * 2.1f}. Board top/bottom edges
computed symmetrically: \texttt{boardBottom = size.height - uy(BRD\_H)},
\texttt{boardTop = size.height - boardBottom}. Small inset
\texttt{r * 0.1f} from border.

\paragraph{Coordinate System Fix}

\texttt{onSizeChanged} on the Canvas captures the actual rendered canvas
pixel dimensions into \texttt{canvasW}/\texttt{canvasH}. The
\texttt{pointerInput} tap handler uses these instead of
\texttt{size.width/height} (which reflects the Box size and differs
from Canvas size on tablets with non-square layouts). This fixes Undo,
Commit, Roll, and point tap detection on the Xiaomi Pad 6.

Button tap y-range enlarged from \texttt{boardCY + DIE\_W * 1.5f} to
\texttt{boardCY + DIE\_W * 2.5f} to accommodate the tablet's larger
physical touch area.

\paragraph{Left Panel}

Fixed proportion \texttt{weight(0.18f)} replacing
\texttt{fillMaxWidth(0.25f)}. Font sizes increased to 18sp/16sp.
Status text uses \texttt{weight} layout so board fills remaining space.

\subsubsection{7.3 Architecture Notes}\label{arch-v087}

The coordinate mismatch between \texttt{pointerInput} and
\texttt{Canvas} is a Compose subtlety: both composables report a
\texttt{size} property, but they are measured independently. When a
\texttt{Box} wraps a \texttt{Canvas} with \texttt{fillMaxSize()}, the
Box size includes any padding or weight allocation, while the Canvas
size is the actual drawing area. On devices where the left panel takes
a different proportion of screen width, these differ. The correct
solution is to capture the Canvas size via \texttt{onSizeChanged} and
share it with the gesture handler.

The Xiaomi Pad 6 delivers 3--4 simultaneous touch events per tap from
a single finger contact. This is hardware behaviour. Guards at the
ViewModel level (\texttt{ENGINE\_THINKING} phase, \texttt{AtomicBoolean})
prevent duplicate engine calls. The touch multiplicity is not a bug in
the application.

\subsubsection{7.4 Remaining for v0.8.8}\label{next-v087}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Match length selector UI (1, 3, 5, 7 point) in left panel or settings
\item
  Engine doubles player --- \texttt{CUBE\_OFFERED} phase with Accept/Drop
  buttons (code exists, untested in multi-point match)
\item
  New game within match --- preserve \texttt{ms.anScore}, reset board only
\item
  Cube visual --- show \texttt{cubeValue} on face, move position on
  ownership change (top = engine, bottom = human, centre = available)
\item
  Resign --- \texttt{CommandResign(NULL)} for game or match, with
  confirmation dialog
\item
  16KB page alignment --- rebuild \texttt{.so} with
  \texttt{-Wl,-z,max-page-size=16384} for Android 16
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}
\emph{GNU Backgammon Android Port --- MASTER v0.8.9 --- clavierhaus.at
--- June 2026}

\subsection{8. Version v0.8.9 --- Playability, Dice Semantics, Cube Path, and Touch Convenience}\label{version-v089}

\subsubsection{8.1 Milestone}

Version v0.8.9 consolidates the playable Android prototype after v0.8.7. The focus is not a broad feature expansion, but a correction and usability milestone: dice state, per-die undo, cube handling, and touch shortcuts are made consistent with the architectural rule that GNU Backgammon remains the sole authority for legal play.

\subsubsection{8.2 Delivered}

\begin{itemize}
\tightlist
\item
  \textbf{Start screen:} provisional match setup screen with match length and opponent-strength choices.
\item
  \textbf{Dice display:} used dice are solid muted colours rather than transparent overlays; dice remain in the displayed order unless explicitly swapped by the player.
\item
  \textbf{Per-die undo:} Undo restores the previous submove snapshot rather than resetting the entire human turn.
\item
  \textbf{Unplayable remaining dice:} if GNUbg reports no legal continuation after a submove, impossible remaining dice are consumed visually.
\item
  \textbf{Destination-stack tap:} tapping an empty or opponent destination point can move two legal checkers there at once when the action is unambiguous.
\item
  \textbf{Cube path:} human cube tap is routed through GNUbg cube decision logic and native match-state mutation; \texttt{CommandDouble()} is deliberately avoided in this path.
\item
  \textbf{Board-relative layout discipline:} visual and hitbox changes continue to avoid absolute screen-position constants.
\end{itemize}

\subsubsection{8.3 Architecture Notes}

The most important distinction in v0.8.9 is between \emph{convenience interaction} and \emph{game authority}. Destination-stack tapping, per-die undo, dice greying, and dice swapping are UI conveniences. They are acceptable only because every resulting board state is produced by GNUbg submove calls or restored from an uncommitted UI snapshot.

The cube implementation follows the same rule. The Android app reads the native cube state, asks GNUbg for the cube decision using the real match board, and then applies the accepted/taken double through a native function that mutates GNUbg match state under the existing legal cube window.

\subsubsection{8.4 Remaining for v0.9}

\begin{itemize}
\tightlist
\item
  Proper cube pass/drop handling.
\item
  Beaver UI and native path.
\item
  Real difficulty/strength wiring into GNUbg evaluation settings.
\item
  SGF save/load UI.
\item
  Tutor, analysis, hint, equity, and MWC surfaces.
\item
  Final visual design and broader device QA.
\end{itemize}
