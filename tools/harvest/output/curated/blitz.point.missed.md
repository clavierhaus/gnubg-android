# Curated: blitz.point.missed
Signature (measured; tools/harvest/signatures.py): opp I_ENTER2 up (min
0.10, w1.5) · opp I_BACK_CHEQUER up (min 0.10) · opp I_ENTER up (min 0.10)
· opp PipCount up (min 3) -- the hit-AND-make conjunction; hit.declined's
...
ALL FOUR flags inverted polarity (described the best play) -- the same
systematic defect as board.close.entry's run; rejected wholesale, praise
drafts carry the entry. ACTED ON this time: the harvest system prompt now
contains an explicit polarity example, so the third run should not repeat
it. Two of six generator pairs failed the BACK_CHEQUER gate honestly (a
checker hit on the 2-point barely moves toward the bar); 4 pairs stand.
prime.break.5 co-fires in slot 2 on two pairs -- physically true (the
point also tightens containment) and within the up-to-two contract.
