# Curated: anchor.advance.mid
Signature (measured; tools/harvest/signatures.py): me I_FORWARD_ANCHOR up
(min 0.25), played in [0.01,0.40] (deep anchor), best in [0.45,0.70] (the
21/22-point band; 0.667/0.500 measured). Split out of anchor.advance.golden,
whose earlier [0.5,1.0] range fired the GOLDEN phrase on these advances --
wrong words, now impossible by range. The 19-advance (their 6-point, 1.0)
is deliberately uncovered rather than mislabeled.
...
First polarity-clean run (the prompt example landed). Cut: "keeps you in
the game" (a22f2, over-claims), "a advanced point" (a22f2 praise, typo),
"easier escape route" claims kept in quotes but softened in the proposal
(escape inputs were not part of this signature's measured terms -- the
phrase should not lean on unmeasured mechanics).
