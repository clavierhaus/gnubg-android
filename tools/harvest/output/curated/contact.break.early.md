# Curated: contact.break.early
Signature (measured; tools/harvest/signatures.py): me I_BREAK_CONTACT up (min 0.045); me I_BACK_ANCHOR up (the run collapses the rearmost made point); PipCount.opp 85-130 gate (the lost race).
Drafts: claude-fable-5 in-session, run 2026-07-19, one draft per verified pair
(3 pairs), written against signature deltas plus the full-vector context
section (deltas_all).
...
Canon note: Holding-game canon: keep the anchor, wait for the shot; run only when the race turns (Gammon Press 'Running from a Holding Game': when in doubt, stay and try to win).

## Proposed final phrases (tier: authored)
- phrase_flag: That far behind, the race is not your game -- the anchor was; running gives up the shot you were waiting for.
- phrase_praise: The anchor holds -- behind in the race, the holding game and its shot remain yours.

## Reasons (from the data)
- pip counts 139-141 vs 103: the straight race is lost
- me.I_BACK_ANCHOR +0.29: the run collapses the rearmost made point from the anchor to the midpoint
- me.I_BREAK_CONTACT +0.054..+0.060: holding keeps the engagement the shot depends on
