...
# stubs them. Needs: gcc, libglib2.0-dev. Output: tools/narrator/quorum/quorum_harness
set -e
cd "$(dirname "$0")/../../.."
...
SRCS="engine-core/eval.c \
      ...
      tools/narrator/quorum/quorum_harness.c \
      tools/narrator/quorum/quorum_stubs.c"

for f in $SRCS; do
    gcc -c -O1 -DHAVE_CONFIG_H $INC $GLIB "$f" -o "$TMP/$(basename "$f" .c).o"
done
gcc "$TMP"/*.o -lm $(pkg-config --libs glib-2.0) -o "$OUT/quorum_harness"
rm -rf "$TMP"
echo "built $OUT/quorum_harness"
