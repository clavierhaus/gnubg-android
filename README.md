...
<p align="center">
  <img src="docs/screenshots/hub.png" width="88%" alt="Home hub — Play, Train, Review">
</p>

> **Status: 0.21.1.** Three modes — **Play, Train, Review** — are built and working, and the core is stable. The active frontier is deeper analysis reporting and online play. See the [Roadmap](#roadmap) and [`CHANGELOG.md`](CHANGELOG.md).
...
**Editions** — This free edition is complete for playing, training and reviewing your own matches, with gnubg's full evaluations throughout, and it will always remain free software on F-Droid. A **Plus edition** is in preparation for the working player: a free-position editor (build any position, paste a GNU BG ID or XGID from a forum, book or another app) and the explanatory coaching layer. The GNU BG ID of any position remains visible and copyable here.
...
Two apps cover this ground for most players today. This port is free, open, offline, and does match save and reviewed playback on gnubg's own engine:

- **XG Mobile** has the position editor it is chiefly used for, but that editor is widely described as fiddly and the app has grown hard to install on current Android. This port targets current Android; its position editor ships in the forthcoming Plus edition.
- **Backgammon NJ** shows a position's GNU BG ID, top moves, and cube decision during play, and its paid package steps through matches — but it has no position editor. This port shows and exports GNU BG IDs during play; pasted-ID import arrives with the Plus edition.
